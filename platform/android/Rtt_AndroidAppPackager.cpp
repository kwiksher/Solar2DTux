...
//	fSupportsOrientationChange( false )
	...
	AndroidAppPackager::Build( AppPackagerParams * params, const char * tmpDirBase )
	{
		...
		std::string tmpPath(LinuxFileUtils::GetStartupPath(NULL));
		tmpPath.append("/Resources/template_android.zip");

		std::string extractTemplateCmd("unzip ");
		extractTemplateCmd.append(tmpPath.c_str());
		extractTemplateCmd.append(" -d ");
		extractTemplateCmd.append(tmpDir);
		extractTemplateCmd.append("/");
		system(extractTemplateCmd.c_str());
#else
		success = mkdir(mktemp(tmpDir));
#endif
		...
		if (success)
		{
			...
			if (inputFile) //offline build
			{
				...
				{
#ifdef Rtt_LINUX_ENV
					gradleGo.append(" JAVA_VERSION=1.8 gradle");
#else
...
/// @param index The index to the "settings" table in the Lua stack.
	...
	{
		lua_State *L = fVM;
		lua_getglobal( L, "androidCreateProperties" );
